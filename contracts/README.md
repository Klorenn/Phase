...
El WASM actual concatena **`?c=<contract_id>`** al final de esa URL (strkey `C…` de la propia instancia). Así, cuando Freighter hace GET al JSON en **phasee.xyz** (o tu dominio), el backend sabe **qué contrato** simular aunque `NEXT_PUBLIC_PHASE_PROTOCOL_ID` en el servidor apunte a otro despliegue. Sin ese query, solo el default de env encaja con el metadata.

**`token_metadata` (solo lectura, Soroban):** el contrato también expone `token_metadata(token_id: u64) -> Map<Symbol, String>` con las mismas claves lógicas (`name`, `description`, `image`) derivadas **solo del estado on-chain** (sin HTTP en el host). Sirve como complemento para exploradores, simulación o wallets que lean el mapa. La fuente **canónica para clientes** que siguen el flujo [SEP-0050](https://github.com/stellar/stellar-protocol/blob/master/ecosystem/sep-0050.md) sigue siendo **`token_uri`** (HTTPS) y el GET al JSON. Si `image` en cadena está vacío, el endpoint HTTP puede seguir devolviendo una URL absoluta (p. ej. OG / sitio); eso es esperable y no implica “SEP-34” para metadata por URI — en documentación usemos **SEP-0050 + `token_uri` → JSON**.
